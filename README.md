...
```bash
# psql로 접속 (기본 데이터베이스)
docker exec -it postgresql psql -U postgres -d auto_node
...
# 접속
psql -h localhost -p 5432 -U postgres -d auto_node
```
...
1. 서버 추가
   ...
   - Host: `localhost`
   - Port: `5432`
   - Maintenance database: `auto_node`
   - Username: `postgres`
   - Password: `.env`의 `POSTGRES_PASSWORD`
...
docker exec postgresql pg_dump -U postgres auto_node > backup.sql

# 압축 백업
docker exec postgresql pg_dump -U postgres auto_node | gzip > backup.sql.gz

# 커스텀 포맷 백업 (복원 시 더 유연함)
docker exec postgresql pg_dump -U postgres -Fc auto_node > backup.dump

# 특정 테이블만 백업
docker exec postgresql pg_dump -U postgres -t users auto_node > users_backup.sql

# 스키마만 백업 (데이터 제외)
docker exec postgresql pg_dump -U postgres -s auto_node > schema_only.sql

# 데이터만 백업 (스키마 제외)
docker exec postgresql pg_dump -U postgres -a auto_node > data_only.sql
```

#### 데이터베이스 복원
```bash
# SQL 파일 복원
cat backup.sql | docker exec -i postgresql psql -U postgres -d auto_node

# 압축 파일 복원
gunzip -c backup.sql.gz | docker exec -i postgresql psql -U postgres -d auto_node

# 커스텀 포맷 복원
docker exec -i postgresql pg_restore -U postgres -d auto_node < backup.dump
...
# 컨테이너 내부에서 SQL 실행
docker exec postgresql psql -U postgres -d auto_node -f /tmp/backup.sql
```
...
# 4. 백업 파일 복원
gunzip -c backup_20240101_020000.sql.gz | docker exec -i postgresql psql -U postgres -d auto_node

# 5. 복원 확인
docker exec -it postgresql psql -U postgres -d auto_node -c "\dt"
```
...
# 데이터베이스 VACUUM
docker exec postgresql psql -U postgres -d auto_node -c "VACUUM FULL;"
```
...
GRANT CONNECT ON DATABASE auto_node TO readonly;
...
GRANT CONNECT ON DATABASE auto_node TO appuser;
